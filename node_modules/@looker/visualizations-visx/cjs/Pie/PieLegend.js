...
const PieLegend = ({
  ...
  const legendOrientation = position === 'top' || position === 'bottom' ? 'horizontal' : 'vertical';
  ...
  const pageSize = legendOrientation === 'horizontal' ? containerWidth * 0.9 : containerHeight * 0.9;
  const totalPages = legendOrientation === 'horizontal' ? Math.floor(contentWidth / Math.max(pageSize, 1)) : Math.floor(contentHeight / Math.max(pageSize, 1));
